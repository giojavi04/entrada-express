...
import { navigation } from '@/application/config/const';
import { classNames } from '@/application/helpers/classNames';

import LogoText from '../LogoText';

const user = {
  name: 'Javier Montalvo',
  email: 'giojavier@javiermontalvo.me'
};
...
const Navbar = () => {
  return (
    <Disclosure as="nav" className="bg-dianne-950">
      {({ open }) => (
        <>
                  ...
                  <div className="ml-10 flex items-baseline space-x-4">
                    {navigation.map((item) => (
                      <a
                        key={item.name}
                        href={item.href}
                        className={classNames(item.current ? 'bg-dianne-900 text-white' : 'text-gray-200 hover:bg-dianne-900 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium')}
